Tipically the blocks from the [[block cipher]] goes encripting every round, and each round need their own key. These keys procedes from a master key, which is spliting in chunks for use it in the rounds.
